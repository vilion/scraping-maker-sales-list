...
#count = 0
File.open("#{ARGV[0]}内の不動産管理会社の会社リスト.csv", 'w') do |file|
  company_name_list = []
  ...
  select_pref.find("option[value='13']").select_option
  ...
  loop do
    ...
    (0..size-1).each do | index |
      begin
        ...
        file.puts([company_name, company_name_pron, address, postal_code, owner_name, owner_name_pron, registration_number].to_csv)
        file.flush()
    ...
    end while next_btn.nil?

    break if next_btn["onclick"] == ""
    next_btn.click
  end
